$(function(){
	...
	function onComplete(data){
		...
		if(data.misMatchPercentage == 0){
			...
			$('#mismatch').text(100-data.misMatchPercentage);
	...
	(function(){
		...
		xhr.open('GET', 'assets/People.jpg', true);
		...
		xhr2.open('GET', 'assets/People2.jpg', true);
		...
		$('#example-images').click(function(){
			...
			$('#dropzone1').html('<img src="assets/People.jpg"/>');
			$('#dropzone2').html('<img src="assets/People2.jpg"/>');

			$.when(done, dtwo).done(function(file, file1){
				if (typeof FileReader === 'undefined') {
					resembleControl = resemble('assets/People.jpg').compareTo('assets/People2.jpg').onComplete(onComplete);
				} else {
					resembleControl = resemble(file).compareTo(file1).onComplete(onComplete);
				}
			});
